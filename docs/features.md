...
| M-08 | プロンプト確認・編集 | 議事録・ととのえ生成時のシステムプロンプト/ユーザープロンプトを確認・編集可能 | **v1.3（実装済み）** |
...
| F-06 | ガジェット風コンパクト表示 | 録音中にコンパクトなガジェットウィンドウで常に最前面に表示、レベルメーター・経過時間・操作ボタンを表示、最小化でトレイ格納 | **v1.5（実装済み）** |
| F-07 | 会議アプリ自動検知 | Zoom / Teams / Google Meet / Webex のプロセスを5秒ポーリングで監視し、会議開始をバルーン通知。クリックで録音自動開始。会議終了時に停止通知。設定でON/OFF切替可能 | **v1.6（実装済み）** |
